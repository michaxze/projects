class Business::MessagesController < ApplicationController
  ...
  def index
    ...
    @page_title = "Home - Message Box - "+@place.name.titleize rescue ''
  ...
  def show
    if has_permission?
      ...
      flash[:error] = "Sorry! You dont have permission to view this message."
      redirect_to business_messages_path
    end
		msgSubj = @message.subject.titleize rescue ''
		msgFrom =  @message.sender.name_or_email.titleize  rescue ''
		@page_title ="#{msgSubj} from #{msgFrom}"
